...
# --- Função que buscará o arquivo jsom informe o caminha da pasta onde o arquivo esteja ---
# - - Basta salvar o camihno como TXT e salvar na mesma pasta do arquivo Jsom
def obter_caminho_json(setor=None):
    arquivo_config = "config.txt"
    # Pasta base onde os arquivos ficarão
    diretorio_base = r"C:\PainelJson"
    
    # Se não existir a pasta, você pode decidir criar ou usar o padrão
    if not os.path.exists(diretorio_base):
        diretorio_base = tempfile.gettempdir() # Fallback para pasta temporária se C: falhar
...
# Agora a variável usa a função para decidir o caminho
ARQUIVO_DADOS_PAINEL = obter_caminho_json()
...
class AppChamada(ctk.CTk):
    ...
    # --- TELA DE ACESSO (Obrigatória para definir o Setor) ---
    def tela_setup(self):
        for widget in self.main_frame.winfo_children(): 
            widget.destroy()

        ctk.CTkLabel(self.main_frame, text="* Informações Obrigatórias", font=("Roboto", 24, "bold"), text_color="#fa0808").pack(pady=20)
        
        # Novo campo: Seleção de Setor
        ctk.CTkLabel(self.main_frame, text="Informe o Setor Corretamente:", font=("Roboto", 14, "bold")).pack()
        self.cb_setor = ctk.CTkComboBox(self.main_frame, values=["HOSPITAL-HRAN","PS-CG", "PNEUMOLOGIA","OFTALMOLOGIA","RADIOLOGIA","BARIATRICA", "PRONTO-SOCORRO","CLASSIFICAÇÃO", "TRIAGEM", "AMBULATÓRIO","RECEPÇÃO","RECEPÇÃO 1","MARCAÇÃO","CENTRO-CIRURGICO",], width=350, font=("Roboto", 18))
        ...
        self.ent_sala = ctk.CTkEntry(self.main_frame, placeholder_text="Ex: Consultório 8", width=350, font=("Roboto", 20))
        ...
        ctk.CTkRadioButton(frame_genero, text="Doutor (Dr.)", variable=self.var_genero, value="o", font=("Roboto", 14, "bold"), text_color="#0834fa").pack(side="left", padx=10)
        ctk.CTkRadioButton(frame_genero, text="Doutora (Dra.)", variable=self.var_genero, value="a", font=("Roboto", 14, "bold"), text_color="#fa08e6").pack(side="left", padx=10)
    ...
    def confirmar_setup(self):
        ...
        # Define o caminho do JSON baseado no setor escolhido
        self.arquivo_setor_especifico = obter_caminho_json(self.setor_atual)
        
        # --- TRAVA DE SEGURANÇA ---
        # Verifica se o setor é o padrão ou se está em branco
        if self.setor_atual == "HOSPITAL-HRAN" or not self.setor_atual:
            messagebox.showwarning("Setor Inválido", "Por favor, selecione um SETOR específico (Ex: PS-CG, RADIOLOGIA) para continuar.")
            return
        # -------------------------------
    ...
    def tela_atendimento(self):
        ...
        ctk.CTkButton(frame_import, text="📂 Buscar Lista de pacientes", command=self.importar_lista, width=100, fg_color="#fb8500").pack(side="left", padx=5)
        ...
        # Histórico Admin
        ctk.CTkLabel(self.main_frame, text="Histórico de Atendimentos", font=("Roboto", 16, "bold"), text_color="#121312").pack(anchor="w", padx=40)
        ...
        ctk.CTkButton(frame_rodape, text="📑 Relatório do dia", width=80, command=self.gerar_excel, fg_color="#5a5a5a").pack(side="left")
    ...
    def acao_chamar(self):
        ...
        if not self.salvar_chamada_global(senha, nome):
            return # Para a execução aqui se houver erro de rede
        # -----------------------------
        ...
        threading.Thread(target=self.tocar_sinal_e_anunciar, args=(senha, nome)).start()
